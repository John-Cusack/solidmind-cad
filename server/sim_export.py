...
``SimModel`` (links + joints), the shape both the canonical package manifest
and the URDF are written from.
...
- ``server.sim_package_manifest`` serializes the same model to ``manifest.json``.

**Vendor dialects are not written here.** URDF is the one courtesy format core
emits (consumed by several engines and most third-party tools); SDF, MJCF and
USD are compiled by the engine that consumes them, from the canonical package
(architecture doc, Principle 3).  Gazebo's compiler lives in
``gazebo_bridge/package_to_sdf.py``.
"""
...
class CollisionShape:
    ...
    When set on a :class:`SimLink`, the shape is carried into the package
    manifest so the consuming engine can emit a primitive collision instead
    of reusing the visual mesh.

    Mesh-based collision is the fallback for links that don't set this field,
    but it triggers DART/ODE crashes in Gazebo Harmonic on complex shapes
    (e.g. multi-blade propellers).  Drone exports via
    :class:`server.airframes.MulticopterAirframe` always populate a primitive
    shape here.
...
class SimLink:
    ...
    """Optional primitive collision geometry.  Carried into the manifest so
    the engine emits a ``<box>``/``<cylinder>`` collision instead of reusing
    the visual mesh — avoids DART/ODE crashes on complex shapes (multi-blade
    props, high-triangle-count chassis meshes, etc.)."""
