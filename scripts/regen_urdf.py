    ...
    manifest.append(
        {
            ...
            "placement": {
                "position": [0.0, 0.0, round(CHASSIS_Z, 2)],
        ...
        manifest.append(
            {
                ...
                "placement": {"position": coxa_pos, "rotation_quat": coxa_quat},
        ...
        manifest.append(
            {
                ...
                "placement": {"position": femur_pos, "rotation_quat": femur_quat},
        ...
        manifest.append(
            {
                ...
                "placement": {"position": tibia_pos, "rotation_quat": tibia_quat},
        ...
        manifest.append(
            {
                ...
                "placement": {"position": coxa_pos, "rotation_quat": servo_yaw_quat},
        ...
        manifest.append(
            {
                ...
                "placement": {"position": femur_pos, "rotation_quat": servo_pitch_quat},
        ...
        manifest.append(
            {
                ...
                "placement": {"position": tibia_pos, "rotation_quat": servo_pitch_quat},
                "bbox_mm": [39.77, 39.38, 24.0],
                "volume_mm3": 18432.0,
            }
        )
...
for leg_id, suffix in zip(LEG_IDS, LEG_SUFFIXES, strict=False):
    ...
    joints.append(
        JointEdge(
            ...
            joint_type=JointType.REVOLUTE,
            parent_part=f"coxa_{suffix}",
            child_part=f"femur_{suffix}",
            axis=(0, 1, 0),
            origin=(0, 0, 0),  # pitch around local Y
        )
    )
    joints.append(
        JointEdge(
            ...
            joint_type=JointType.REVOLUTE,
            parent_part=f"femur_{suffix}",
            child_part=f"tibia_{suffix}",
            axis=(0, 1, 0),
            origin=(0, 0, 0),  # pitch around local Y
        )
    )
...
for j in sim_model.joints:
    if "coxa" in j.name or "femur" in j.name or "tibia" in j.name:
        print(
            f"  {j.name}: xyz=({j.origin_xyz[0]:.4f}, {j.origin_xyz[1]:.4f}, {j.origin_xyz[2]:.4f})"
            f"  rpy=({j.origin_rpy[0]:.4f}, {j.origin_rpy[1]:.4f}, {j.origin_rpy[2]:.4f})"
        )

# Write URDF
output_path = str(
    Path(__file__).resolve().parent.parent / "hexapod_full_pkg" / "hexapod_18dof_full.urdf"
)
