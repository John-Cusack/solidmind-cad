...
MANIFEST = {
    ...
    "Femur_L1": (
        [105.48, 113.01, 157.98],
        [0.8858961147811252, -0.10315199541276454, 0.23737539474853156, 0.38496839042452735],
    ),
    "Femur_L2": (
        [0.0, 127.0, 157.98],
        [0.6830124833676485, -0.18301289796097248, 0.18301289796097248, 0.6830128153437613],
    ),
    "Femur_L3": (
        [-105.48, 113.01, 157.98],
        [0.38496794074669644, -0.23737480950244239, 0.10315218668510899, 0.8858964447343762],
    ),
    "Femur_R1": (
        [105.48, -113.01, 157.98],
        [0.8858961147811252, 0.10315199541276454, 0.23737539474853156, -0.38496839042452735],
    ...
    "Tibia_L1": (
        [144.48, 154.8, 124.98],
        [0.751283139875577, -0.22859789377779857, 0.5260541291875838, 0.32647159120471936],
    ),
    "Tibia_L2": (
        [0.0, 184.16, 124.98],
        [0.5792278211125137, -0.40557985126140306, 0.40557985126140306, 0.579228020515933],
    ),
    "Tibia_L3": (
        [-144.48, 154.8, 124.98],
        [0.32647151275243197, -0.5260542180192672, 0.22859840306596504, 0.7512829568018742],
    ),
    "Tibia_R1": (
        [144.48, -154.8, 124.98],
        [0.751283139875577, 0.22859789377779857, 0.5260541291875838, -0.32647159120471936],
    ...
    "Servo_hip_pitch_L1": (
        [105.48, 113.01, 157.98],
        [0.36670501549791396, 0.0, 0.0, 0.9303372676662344],
    ),
    ...
    "Servo_hip_pitch_L3": (
        [-105.48, 113.01, 157.98],
        [-0.36670501549791407, 0.0, 0.0, 0.9303372676662344],
    ),
    "Servo_knee_L3": (
        [-144.48, 154.8, 124.98],
        [-0.36670501549791407, 0.0, 0.0, 0.9303372676662344],
    ...
    "Servo_hip_yaw_R1": (
        [70.0, -75.0, 157.98],
        [-0.9171473939110514, 0.0, 0.0, 0.3985481876036659],
    ),
    "Servo_hip_pitch_R1": (
        [105.48, -113.01, 157.98],
        [0.9303372676662345, 0.0, 0.0, 0.36670501549791384],
    ),
    "Servo_knee_R1": (
        [144.48, -154.8, 124.98],
        [0.9303372676662345, 0.0, 0.0, 0.36670501549791384],
    ...
    "Servo_hip_yaw_R3": (
        [-70.0, -75.0, 157.98],
        [-0.3985481876036655, 0.0, 0.0, 0.9171473939110515],
    ),
    "Servo_hip_pitch_R3": (
        [-105.48, -113.01, 157.98],
        [-0.9303372676662345, 0.0, 0.0, 0.36670501549791384],
...
def transform_stl(stl_path: Path, position: list[float], quat: list[float]) -> None:
    ...
    vertex_re = re.compile(r"(\s+vertex\s+)([\d.e+-]+)\s+([\d.e+-]+)\s+([\d.e+-]+)")
    ...
    normal_re = re.compile(r"(\s+facet normal\s+)([\d.e+-]+)\s+([\d.e+-]+)\s+([\d.e+-]+)")
