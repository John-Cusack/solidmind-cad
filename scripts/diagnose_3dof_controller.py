...
print(f"  l_coxa={geom.l_coxa*1000:.1f}mm  l_femur={geom.l_femur*1000:.1f}mm  l_tibia={geom.l_tibia*1000:.1f}mm")
print(f"  body_length={config.body_length*1000:.1f}mm  body_width={config.body_width*1000:.1f}mm")
...
for i, name in enumerate(LEG_NAMES):
    ...
    print(f"\n  {name}:")
    print(f"    mount: ({mount.x*1000:.1f}, {mount.y*1000:.1f}) mm, angle={DEG(mount.angle):.1f}°")
    print(f"    default foot (body frame): ({foot[0]*1000:.1f}, {foot[1]*1000:.1f}, {foot[2]*1000:.1f}) mm")
    print(f"    standing bias: coxa={DEG(bias[0]):.2f}°  femur={DEG(bias[1]):.2f}°  tibia={DEG(bias[2]):.2f}°")
    ...
    dist = math.sqrt((hip_pt[0]-fk_pt[0])**2 + (hip_pt[1]-fk_pt[1])**2 + (hip_pt[2]-fk_pt[2])**2)
    print(f"    hip frame target: ({hip_pt[0]*1000:.1f}, {hip_pt[1]*1000:.1f}, {hip_pt[2]*1000:.1f}) mm")
    print(f"    IK solution: coxa={DEG(angles.coxa):.2f}°  femur={DEG(angles.femur):.2f}°  tibia={DEG(angles.tibia):.2f}°")
...
for tick in range(3):
    ...
    for i, name in enumerate(LEG_NAMES):
        ...
        print(f"    {name}: coxa={DEG(vals[0]):+7.2f}°  femur={DEG(vals[1]):+7.2f}°  tibia={DEG(vals[2]):+7.2f}°")
