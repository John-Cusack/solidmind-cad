...
class TestCadExportSimPackage(unittest.TestCase):
    ...
    @patch("server.motion_store.get")
    @patch("server.tools_cad.get_client")
    def test_export_emits_no_vendor_artifacts(
        self, mock_get: MagicMock, mock_mech_get: MagicMock
    ) -> None:
        """Core writes manifest + meshes + URDF — never SDF or PX4 scripts.

        Both are compiled by the gazebo bridge from the package at load time
        (``gazebo_bridge/package_to_sdf.py``, ``gazebo_bridge/px4_airframe.py``).
        """
        import json
        import tempfile
        ...
        result = cad_export_sim_package(
            mechanism_id="mech_drone",
            drone_config={
                "rotors": [{"index": 0, "joint": "rotor_fl_joint", "direction": "ccw"}],
                "px4": True,  # ignored by core — the bridge owns PX4 artifacts
            },
        )
        self.assertTrue(result["ok"])
        self.assertIn("urdf_path", result)
        self.assertIn("manifest_path", result)
        for vendor_key in ("sdf_path", "sdf_validation", "airframe_id", "airframe_path"):
            self.assertNotIn(vendor_key, result)
        self.assertEqual(
            [p for p in Path(tmp_dir).glob("*.sdf")],
            [],
            "core must not write any SDF into the package",
        )

        # The rotor survives as an abstract actuator spec in the manifest.
        with open(result["manifest_path"]) as fh:
            manifest = json.load(fh)
        actuator = manifest["actuators"][0]
        self.assertEqual(actuator["type"], "rotor")
        self.assertEqual(actuator["link"], "rotor_fl")
        self.assertEqual(actuator["direction"], "ccw")
        self.assertGreater(actuator["max_thrust_N"], 0.0)
