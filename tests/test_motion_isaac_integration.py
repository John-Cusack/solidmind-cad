"""Integration tests for motion tools with the Isaac backend.
...
class _FakeIsaacBridge:
    """Small in-process TCP server that emulates the Isaac bridge protocol."""
            ...
            return {
                ...
                "result": {
                    "time_series": [
                        ...
                        {"t": duration, "parts": {pid: {"omega_rpm": 123.4} for pid in part_ids}},
                    ],
                    "summary": {
...
class TestMotionIsaacIntegration(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.bridge.start()
        isaac_client._client = isaac_client.IsaacClient(host="127.0.0.1", port=self.bridge.port)  # type: ignore[attr-defined]
    ...
    def test_batch_simulate_default_backend_routes_to_isaac(self) -> None:
        ...
        result = mcp_main._call_tool(
            "motion.simulate",
            {"mechanism_id": defined["mechanism_id"], "duration_s": 0.25},
        )
    ...
    def test_full_teleop_lifecycle_via_main_dispatch(self) -> None:
        ...
        started = mcp_main._call_tool(
            "motion.teleop_start",
            {"mechanism_id": mechanism_id, "backend": "isaac"},
        )
        ...
        self.assertAlmostEqual(state["state"]["vx_mps"], 0.35)
        self.assertAlmostEqual(state["state"]["yaw_rate_rps"], 0.7)
        self.assertAlmostEqual(state["state"]["body_height_m"], 0.04)
    ...
    def test_unavailable_isaac_returns_choice_contract(self) -> None:
