...
def _make_spec(run_dir: Path):
    ...
    run.spec.objectives = [
        Objective(
            name="ratio_match", direction="maximize", unit="ratio", weight=1.0, threshold=0.99
        ),
        Objective(name="mass", direction="minimize", unit="kg", weight=0.5, threshold=0.5),
    ]
    ...
    run.spec.skeleton = AssemblySkeleton(
        ...
        reserved_volumes={
            "gearbox_envelope": {
                "origin": [0, 0, 0],
                "size": [
                    PITCH_DIAMETER_RING_MM + 8,
                    PITCH_DIAMETER_RING_MM + 8,
                    max(GEAR_THICKNESS_MM, RING_THICKNESS_MM, CARRIER_THICKNESS_MM + PIN_HEIGHT_MM)
                    + 2,
                ],
            },
        },
    ...
    interfaces = [
        Interface(
            ...
            validation=ValidationMethod(
                check_points=[
                    ValidationCheckPoint(
                        feature="bore_dia", expected_mm=SHAFT_BORE_MM, tolerance_mm=0.2
                    ),
                ]
            ),
        ),
        ...
        interfaces.append(
            Interface(
                ...
                validation=ValidationMethod(
                    check_points=[
                        ValidationCheckPoint(
                            feature="bore_dia", expected_mm=PIN_BORE_MM, tolerance_mm=0.2
                        ),
                    ]
                ),
            )
        ...
        Interface(
            ...
            validation=ValidationMethod(
                check_points=[
                    ...
                    ValidationCheckPoint(
                        feature="pin_circle_dia", expected_mm=2 * ORBIT_RADIUS_MM, tolerance_mm=0.5
                    ),
                ]
            ),
        )
    ...
    subsystems.append(
        Subsystem(
            ...
            name="ring_gear",
            kind=SubsystemKind.GENERATED,
            envelope_mm=[PITCH_DIAMETER_RING_MM + 6, PITCH_DIAMETER_RING_MM + 6, RING_THICKNESS_MM],
            mass_budget_kg=0.20,
            material="steel",
            interfaces=["ifc_ring"],
            worker_count=1,
    ...
    interfaces.append(
        Interface(
            ...
            validation=ValidationMethod(
                check_points=[
                    ValidationCheckPoint(
                        feature="bore_dia", expected_mm=ring_root_dia_mm, tolerance_mm=0.5
                    ),
                ]
            ),
        )
...
def main() -> int:
    ...
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("/tmp/planetary_gearbox_run"),
    ...
    print(
        f"Planetary gearbox 5:1 — module={MODULE_MM}, sun={SUN_TEETH}, "
    ...
    if not common.freecad_ready():
        print(
            f"ERROR: FreeCAD addon not reachable at "
    ...
    for r in reports:
        ...
        for dc in r.dimension_checks:
            ...
            print(
                f"        {mark} {dc.feature}: measured={dc.measured_mm}, "
